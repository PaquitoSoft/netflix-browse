...
const Showcase: React.FC<ShowCaseProps> = props => {
	...
	if (data.results) {
		...
		titles = slice.map(title => {
			const movieTileProps: MovieTileProps = createMovieTitleProps(title, props);
...
function createMovieTitleProps(title: ApiDataEntry, props: ShowCaseProps): MovieTileProps {
	return ({
